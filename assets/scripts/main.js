...
const recipes = [
  ...
  'assets/recipes/AppleCake.json',
  'assets/recipes/ApplePie.json',
  'assets/recipes/BlueberryPie.json',
];
...
function createRecipeCards() {
  ...
  for (let key=0;key<recipes.length;key++){
    ...
    recipecard.data=recipeData[recipes[key]];
...
function Hide() {
  ...
  for (let card of cards) {
    if (i >= cardsShown) {card.style.display = "none";}
    i++;
  }
}
...
function Show() {
  ...
  let i=0;
  for (let card of cards) {
    if (i >= cardsShown) {card.style.display = "";}
    i++;
  }
}